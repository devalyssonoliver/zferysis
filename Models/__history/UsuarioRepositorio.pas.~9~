...
uses
  System.SysUtils, IUsuarioRepositorio, Usuario, FireDAC.Comp.Client,
...
function TUsuarioRepository.Editar(const Codigo: Integer;
...
begin
  ...
  try
    Query.Connection := GerenciadorConexao.fdConn;
    Query.SQL.Text :=
      'UPDATE usuarios SET nome = :nome, senha = :senha, ativo = :ativo WHERE codigo = :codigo';
    Query.Params.ParamByName('nome').AsString := Nome;
    Query.Params.ParamByName('senha').AsString := Senha;
    Query.Params.ParamByName('ativo').AsBoolean := Ativo;
